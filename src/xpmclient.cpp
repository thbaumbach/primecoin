...
void PrimeMiner::InvokeMining(void *args/*, zctx_t *ctx, void *pipe*/) {
	...
	((PrimeMiner*)args)->Mining(NULL /*ctx, pipe*/);
	
}
//<xolominer/>
//void PrimeMiner::Mining(/*zctx_t *ctx, void *pipe*/) {
void PrimeMiner::Init(/*zctx_t *ctx, void *pipe*/) {
	/*
...
#define MFS 2*SW*MSO	// max fermat size
	...
	*/
	...
	error = 0;
	primeBuf = clCreateBuffer(gContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
										mConfig.PCOUNT*sizeof(cl_uint), &gPrimes[mPrimorial+1], &error);
	OCL(error);
	primeBuf2 = clCreateBuffer(gContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
...
void PrimeMiner::Mining(CBlockProvider* bp /*zctx_t *ctx, void *pipe*/) {
	...
	while(run)
	{
		...
		*/		
		// reset if new work
		if((void*)work != (void*)bp->getOriginalBlock()){ //TODO: this
			work = (block_t*)bp->getOriginalBlock();
			if (work == NULL)
				continue;
			else
				work_data = *bp->getOriginalBlock();
			...
			*/
			blockheader.hashPrevBlock = work->hashPrevBlock;
			blockheader.hashMerkleRoot = work->hashMerkleRoot;
			blockheader.time = work->time + mID;
			blockheader.bits = work->bits;
			blockheader.nonce = 1;
		...
		// sieve dispatch
		{
			...
			for(int i = 0; i < SW; ++i){
				...
				if(!hashlist.size()){
					...
					//	printf("warning: ran out of hashes. pipeline stalled.\n");
					break;
				}
		...
		if(candis.size()) {
			...
			for(unsigned i = 0; i < candis.size(); ++i){
				...
				if(chainlength >= 7 /*block.minshare()*/){
					...
					*/
					...
					work_data.nNonce = hash.nonce;
					work_data.bnPrimeChainMultiplier.SetHex(sharemulti.get_str(16));
					
					printf("GPU %d found share: %d-ch type %d\n", mID, chainlength, candi.type+1);
					if(isblock)
						printf("GPU %d found BLOCK!\n", mID);
					
					//Send(share, sharepush); //TODO: !!!!!!!!!!!!!!!!!!!!!!!!!
					bp->submitBlock(&work_data, 0);
...
bool XPMClient::Initialize(size_t gpu_num_to_use /*Configuration* cfg*/) {
	...
	int cpuload = 1; //TODO: cfg->lookupInt("", "cpuload", 1);
	int depth = 5 - cpuload;
